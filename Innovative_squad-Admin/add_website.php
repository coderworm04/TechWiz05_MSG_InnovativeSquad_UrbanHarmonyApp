...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // Prepare and bind
        $stmt = $pdo->prepare("INSERT INTO tbl_website (website_name, user_id) VALUES (:website_name, :user_id)");
        $stmt->bindParam(':website_name', $website_name);
        $stmt->bindParam(':user_id', $user_id);

        // Execute the statement
        if ($stmt->execute()) {
            echo "<script>alert('Website added successfully')</script>";
            
        } else {
            echo "<script>alert('Error adding Website.')</script>";
        }
            ...
            <nav class="navbar bg-light navbar-light d-flex flex-column">
                ...
                <div class="navbar-nav w-100">
                    ...
                    if ($_SESSION['role'] == "admin") {
                        echo "
                        <a href='user.php' class='nav-item nav-link text-white fw-bold'><i class='fa text-light fa-user me-2'></i>Users</a>
                        <a href='website.php' class='nav-item nav-link text-white fw-bold'><i class='fa text-light fa-user me-2'></i>User's Website</a>
                        ";
                    }
                    ?>
                </div>
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-light text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h3 class="mx-auto text-white">Add Website</h3>
                    </div>
                    <form class="row g-3" method="POST" action="">
                        
                        <div class="col-md-6">
                            <label for="validationCustom06" class="form-label">Website Name</label>
                            <input type="text" class="form-control" id="validationCustom01" name="website_name" required>
                        </div>
						<div class="col-md-6">
                            <label for="validationCustom07" class="form-label">User</label>
                            <select class="form-select" id="validationCustom02" name="user_id" required>
